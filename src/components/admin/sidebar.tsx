...
export function Sidebar({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn('sticky left-0 right-0 top-0 pb-12 shadow-xl', className)}
    >
      <div className="h-full space-y-4 py-4">
        <div className="flex items-center justify-center">
          <h1 className="text-3xl font-bold text-primary">Rental Cars</h1>
        </div>
        ...
        <div className="h-full px-3 py-2">
          <div className="flex h-full flex-col items-start justify-between">
                  ...
                  <Link
                    ...
                    className={cn(
                      'flex items-center justify-start gap-2 rounded px-4 py-3 transition-colors duration-200' +
                        'hover:bg-slate-100 active:scale-95 dark:bg-slate-700 dark:hover:bg-slate-600',
            ...
            <div className="w-full">
              <Button
                variant="outline"
                className="w-full justify-start bg-slate-50"
                onClick={handleLogout}
              >
                <LogOut className="mr-2 h-4 w-4 rotate-180 text-black dark:text-white" />
                Sign out
              </Button>
            </div>
          </div>
